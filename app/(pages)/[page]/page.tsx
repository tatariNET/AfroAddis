  ...
  return (
    <div className="space-y-12">
      <section className="rounded-[32px] border border-white/60 bg-white/80 p-8 shadow-xl shadow-gray-900/5 sm:p-12">
        <p className="text-xs font-semibold uppercase tracking-[0.3em] text-gray-500">{page.kicker}</p>
        <h1 className="mt-4 text-4xl font-semibold tracking-tight text-gray-900">{page.title}</h1>
        <p className="mt-4 max-w-2xl text-base text-gray-600">{page.description}</p>
        {page.stats && (
          <dl className="mt-8 grid gap-6 text-sm text-gray-700 sm:grid-cols-3">
            {page.stats.map((stat) => (
              <div key={stat.label} className="rounded-2xl border border-gray-100 bg-white/70 p-4 text-center shadow-sm">
                <dt className="text-xs uppercase tracking-[0.3em] text-gray-500">{stat.label}</dt>
      ...
      {page.sections.map((section) => (
        ...
        >
          <div className="space-y-3">
            <p className="text-xs uppercase tracking-[0.3em] text-gray-500">{section.title}</p>
